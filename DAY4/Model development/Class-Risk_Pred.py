...
# ============================================
# 🚀 Faulty Device Severity Prediction (Colab/RAM-safe)
...
# -----------------------------
devices = pd.read_csv("device_cleaned.csv", encoding="ISO-8859-1", low_memory=False)
manufacturers = pd.read_csv("manufacturer_cleaned.csv", encoding="ISO-8859-1", low_memory=False)
events = pd.read_csv("events_cleaned.csv", encoding="ISO-8859-1", low_memory=False)

# -----------------------------
# 2) Merge: events → devices → manufacturers
# -----------------------------
df = (
...
# ----------------------------------------------------
# 6) Preprocess (impute + encode) → Logistic Regression
...
def predict_device_severity(device_id):
    ...
    if subset.empty:
        return f"❌ Device ID {device_id} not found in merged data."
    ...
    pred = preds[0]
    out = f"✅ Predicted class severity for device_id={device_id}: {pred}"

    if probs is not None:
        classes = clf.named_steps["logisticregression"].classes_
        proba_row = probs[0]

        # risk % = probability of the predicted class
        risk_score = proba_row[list(classes).index(pred)] * 100

        # sorted probabilities
        proba_sorted = sorted(zip(classes, proba_row), key=lambda x: -x[1])
        top = ", ".join([f"{c}: {p*100:.1f}%" for c, p in proba_sorted])

        out += f"\n   🔎 Risk % (confidence in prediction): {risk_score:.1f}%"
        out += f"\n   📊 Probabilities → {top}"
